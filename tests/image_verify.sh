...
process_docker_env
if [ $DEBUG -eq 1 ]; then
  echo "Docker run command: docker run ${DOCKER_ENV} --rm -d -v ${PWD}/tests/html:/var/www ${DOCKER_IMAGE}"
fi
CONTAINER_ID=$(docker run ${DOCKER_ENV} --rm -d -v ${PWD}/tests/html:/var/www ${DOCKER_IMAGE})
if [ $? -ne 0 ]; then
